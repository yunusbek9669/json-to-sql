...
fn auto_prefix_field(field_sql: &str, table_alias: &str) -> String {
    let builtins = vec![
    ...
    let local_aliases: Option<&HashMap<String, String>> = None;
...
fn main() {
    let sqls = vec![
        "EXISTS(SELECT 1 FROM public.category WHERE public.category.parent_id = cat.id)",
        "EXISTS(SELECT 1 FROM public.category c WHERE c.parent_id = cat.id)",
        "EXISTS(SELECT 1 FROM category WHERE category.parent_id = cat.id)",
    ];
    for sql in sqls {
        println!("Input:  {}", sql);
        println!("Output: {}", auto_prefix_field(sql, "cat"));
        println!("---");
    }
}
